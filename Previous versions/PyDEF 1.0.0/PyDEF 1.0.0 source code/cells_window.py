...
class Cells_Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        ttk.Button(self.button_frame, text='Plot DOS', command=plot_dos).grid(row=1, column=0, columnspan=2, padx=5)
...
class Cell_Properties_Window(tk.Toplevel):
    ...
    def display_dos_parameters_window(self):
        ...
        self.dos_type_frame_label = ttk.Label(self, text='DOS displayed', font='-weight bold')
        self.dos_type_frame = ttk.LabelFrame(self.column1_frame, labelwidget=self.dos_type_frame_label)
        ...
        ttk.Label(self.proj_dos_frame, text='Projection').grid(row=3, column=0, sticky='w')
        ttk.Radiobutton(self.proj_dos_frame, text='Orbitals projection', variable=self.tot_proj_dos_var,
                        value=False).grid(row=4, column=0)
        ...
        self.button_frame.pack(side='bottom', expand=True, fill='both')
        ...
        ttk.Button(self.button_frame, text='Save', command=save_parameters).pack(side='right', pady=5, padx=3)
        ttk.Button(self.button_frame, text='Cancel', command=self.destroy).pack(side='right', pady=5, padx=3)
...
class Band_Occupation_Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        for kpoint, row in zip(self.cell.bands_data, range(1, len(self.cell.bands_data) + 1)):
            kpoint_id = self.tree.insert('', row, text='kpoint ' + str(row))
...
class Colors_Choice_Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        def set_default_colors():
            """ Set the  """
            if parent.tot_proj_dos_var.get() is False:
                parent.proj_colors_choice = pc.Dos_Plot_Parameters(parent.cell).colors_proj
            else:
                parent.tot_colors_choice = pc.Dos_Plot_Parameters(parent.cell).colors_tot
            self.destroy()
